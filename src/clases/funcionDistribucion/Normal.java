...
//                        " N°: " + n);
    ...
    public void probabilidad(int tam)
    {
        ...
        {
            marca = (ie.getInferior() + ie.getSuperior())/2;
            p = (Math.pow(Math.E,(-0.5 * (Math.pow(((marca - media)/ desviacion), 2)))) /
                    (desviacion * Math.sqrt(2 * Math.PI))) * (ie.getSuperior() - ie.getInferior());

            //System.out.println(p * tam);
            //ie.setF_Esp((float)Math.round((p * tam)*10000)/10000);
            ie.setF_Esp((float)Math.round((p * tam)*100)/100);
